...
export default function Favourite() {
  ...
  useEffect(() => {
    ...
    if (getArrFromLocal !== "[]" && localStorage.getItem("markedFilms")) {
      getArrFromLocal.forEach((el) => {
        fetch(
          `https://api.themoviedb.org/3/movie/${el}?api_key=82317c5aa540a6308c278894685205da&language=en-US`
        )
          .then((data) => data.json())
          .then((json) => {
  ...
  return (
    <div className={s.container} data-testid="favourite">
      <p onClick={lastPage} className={s.home}>
        &#8735;
      </p>
      <h2>Your Favourite</h2>
      <div className={s.blocks}>
        {favouriteFilms.length !== 0
          ? favouriteFilms.map((el) => <BlockFilm key={el.id} dataFilm={el} />)
          : "It's empty"}
      </div>
    </div>
  );
