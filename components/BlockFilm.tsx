...
interface PropsBlockFilm {
  [key: string]: any;
}

const BlockFilm: FC<PropsBlockFilm> = ({ dataFilm }) => {
  const [activeStar, setActiveStar] = useState(false);
  const changeArrInLocal = (key) => localStorage.setItem("markedFilms", key);

  const changeActeveStar = () => {
    const localData = safeJSONParse<number[]>(
      localStorage.getItem("markedFilms")
    );
    if (activeStar) {
      setActiveStar(false);
      if (localData && localData.length) {
        changeArrInLocal(
          JSON.stringify(
            localData.filter((element) => element !== dataFilm?.id)
          )
        );
      }
    } else {
  ...
  useEffect(() => {
    ...
    localData &&
      localData.forEach((el) => el === dataFilm?.id && setActiveStar(true));
  }, []);

  return (
    <div className={s.blockFilm} data-testid="blockFilm">
      <div
        className={s.img}
        style={
          dataFilm?.backdrop_path && {
            background: `url('https://www.themoviedb.org/t/p/w300_and_h450_bestv2${
              dataFilm?.poster_path && dataFilm.poster_path
            }') center/cover`,
          }
        }
      >
        <div className={s.starLink}>
          <a
            href={`https://www.themoviedb.org/movie/${
              dataFilm?.id && dataFilm.id
            }`}
            data-testid="linkBlockFilm"
          >
            <svg
              fill="none"
              height="24"
          ...
          <div
            data-testid="star"
            className={activeStar ? `${s.star} ${s.active}` : s.star}
            onClick={changeActeveStar}
          >
            <svg
        ...
        <div className={s.rating}>
          <p>{dataFilm?.vote_average && dataFilm.vote_average}</p>
        </div>
      </div>
      <div className={s.text}>
        <h4>{dataFilm?.original_title && dataFilm.original_title}</h4>
        <p>{dataFilm?.release_date && dataFilm.release_date}</p>
      </div>
    </div>
  );
};
